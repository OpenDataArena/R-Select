...
R-Select 在优化前需要为每条数据样本计算多维质量分数。项目将 [ODA-DataScorer](ODA-DataScorer/) 作为子模块引入，提供 **60+ 个 scorer**：
...
> 完整 scorer 列表及参数说明见 [ODA-DataScorer/README.md](ODA-DataScorer/README.md)。

### 步骤 1：定义打分指标

在 `scores.txt` 中列出要使用的指标名（每行一个）。例如 `configs/demo/scores.txt` 定义了 demo 使用的 30 个指标：
...
这些名称将作为数据池 `scores` 字典的键，并在 R-Select 配置中被引用。

### 步骤 2：安装 ODA-DataScorer
...
根据你定义的指标创建 ODA-DataScorer 的 YAML 配置。以下为各类别的示例：
...
**打分结果位置**：ODA-DataScorer 各模块的输出目录下会有 `pointwise_scores.jsonl`（例如 model-based 在 `ODA-DataScorer/model_based/results/<任务名>/pointwise_scores.jsonl`，heuristic也一致，在其结果目录下）。
...
**R-Select 所需格式**：需要把分数合并回原始数据池，每条样本为一条 JSONL，且 `scores` 为「键 → 数值」的扁平字典，例如：

```json
{"id": 0, "instruction": "...", "input": "", "output": "...", "scores": {"AtheneScorer": 4.5, "PPLScorer": 12.34}}
```

因此需要：从 `pointwise_scores.jsonl` 读取打分结果，将嵌套的 `{"score": value}` 展平为数值，按 `id` 合并到原始数据中，得到含 `id`、`instruction`、`input`、`output` 和 `scores` 的 pool 文件。若使用其他方式打分或自定义分数名称，只要保证 **scores.txt**、**R-Select 的输入数据** 与 **R-Select 具体优化 config 中的 score 名称** 三者一致即可。
